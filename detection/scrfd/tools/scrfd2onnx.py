        ...
        if simplify:
            ...
            if dynamic:
                ...
                model, check = simplify(
                    model, input_shapes=input_shapes, dynamic_input_shape=True
                )
            else:
...
"""
python3 tools/scrfd2onnx.py --shape 640 640 --output-file scrfd_2.5g_bnkps_640x640.onnx

pip install torch
pip install onnxsim

pip install -U openmim
mim install mmcv==1.3 -i https://pypi.tuna.tsinghua.edu.cn/simple
# pip 安装/升级的 setuptools 失败 最终 还是 conda install -y setuptools 解决的
...
python -u tools/scrfd2onnx.py ./configs/"$GROUP"/"$TASK".py /root/scrfd_2.5g_bnkps.pth --shape 640 640 --output-file scrfd_2.5g_bnkps_640x640.onnx
"""
